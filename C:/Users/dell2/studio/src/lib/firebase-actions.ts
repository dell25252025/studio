...
export async function getCountryFromCoordinates(latitude: number, longitude: number): Promise<{ country: string | null; error?: string }> {
  try {
    const response = await fetch(
      `https://nominatim.openstreetmap.org/reverse?format=json&lat=${latitude}&lon=${longitude}&accept-language=fr`,
      {
        headers: {
          'User-Agent': 'WanderLinkApp/1.0 (Contact: wanderlink.app/contact)',
        },
      }
    );
    ...
    if (data?.address?.country) {
      return { country: data.address.country };
    } else {
      return { country: null, error: "Pays non trouvé dans la réponse." };
    }
  } catch (error: any) {
    console.error("Error fetching country from coordinates:", error);
    return { country: null, error: error.message || "Une erreur inconnue est survenue." };
  }
}
