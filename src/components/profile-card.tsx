  ...
  return (
    <Link href={`/profile?id=${profile.id}`} passHref>
      <Card className="relative aspect-[3/4] w-full overflow-hidden rounded-2xl border-0 shadow-lg group cursor-pointer">
        ...
        <div className="absolute bottom-0 left-0 p-2 md:p-3 text-white w-full flex flex-col items-start">
          {intention && (
            <div className="mb-1">
              <Badge variant="default" className={cn("border-none text-white text-[10px] h-auto px-2 py-0.5", intention.color)}>
                {intention.text}
              </Badge>
            </div>
          )}
